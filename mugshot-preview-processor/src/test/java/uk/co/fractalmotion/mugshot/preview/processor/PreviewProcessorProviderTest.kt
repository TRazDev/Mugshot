...
class PreviewProcessorProviderTest {
  ...
  @Test
  fun `leaves another processor's generated file alone`() {
    val result = compilation(
      ...
      """,
      otherProcessor = FileGeneratingProcessorProvider
    ).compile()

    assertThat(result.exitCode).isEqualTo(KotlinCompilation.ExitCode.OK)

    // The harness keeps more than one copy of what a processor generates, so every copy is
    // checked rather than assuming there is one.
    val generated = temporaryFolder.root.walkTopDown().filter { it.isFile }.toList()

    val other = generated.filter { it.name == "OtherProcessorOutput.txt" }
    assertWithMessage("the other processor's file was not generated")
      .that(other).isNotEmpty()
    // Mugshot used to write the source set name over whichever file was generated first in the
    // round, which in a real build was Hilt's, leaving the compiler to report the error there.
    other.forEach { assertThat(it.readText()).isEqualTo(OTHER_PROCESSOR_CONTENT) }

    // Mugshot's own marker is still written. Its content is the directory below `ksp` on the
    // path, which is the source set in a real build and something else under this harness, so
    // only its presence is asserted here.
    val ownFile = generated.filter { it.name == "mugshotSourceSet.txt" }
    assertWithMessage("Mugshot's own marker was not generated").that(ownFile).isNotEmpty()
    ownFile.forEach { assertThat(it.readText()).isNotEmpty() }
  }
  ...
  private fun compilation(body: String, otherProcessor: SymbolProcessorProvider? = null): KotlinCompilation {
    ...
    return KotlinCompilation()
      .apply {
        ...
        configureKsp {
          ...
          kspIncremental = true
          // Added first so it generates before Mugshot's, which is the order that used to
          // matter: Mugshot wrote over whichever file was generated first in the round.
          otherProcessor?.let { symbolProcessorProviders += it }
          symbolProcessorProviders += PreviewProcessorProvider()
        }
      }
  }

  /** Generates one file, before Mugshot's processor runs, and never touches it again. */
  private object FileGeneratingProcessorProvider : SymbolProcessorProvider {
      ...
      object : SymbolProcessor {
        ...
        override fun process(resolver: Resolver): List<KSAnnotated> {
          if (!done) {
            ...
            writeFile(environment.codeGenerator)
          }
          return emptyList()
        }
      }

    private fun writeFile(codeGenerator: CodeGenerator) {
      codeGenerator
        .createNewFile(Dependencies(false), "test", "OtherProcessorOutput", "txt")
        .bufferedWriter()
        .use { it.write(OTHER_PROCESSOR_CONTENT) }
    }
  }

  private companion object {
    private const val OTHER_PROCESSOR_CONTENT = "generated by someone else"
