...
### Changed
* A build can ask for a preview to be reported by name, with
  ...
  should be shown as, and the plugin reads it back. Passes and failures read the same way, in the
  words and colours Gradle's own reporting uses. Off by default, since Gradle reports nothing per
  test unless a build asks it to.
